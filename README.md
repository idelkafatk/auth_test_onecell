...
* <a href="#jmeter">Нагрузочное тестирование</a>
...
Созданы тест-кейсы для проверки основных моментов авторизации и аутентификации. Те проверки, которые не счел
нужным описывать в тест-кейсах, включены в чек-лист проверок

<details>
    <summary><h4>Названия тест-кейсов</h4></summary>

>* Страница авторизации. При вводе логина и пароля, имеющихся в системе, происходит  авторизация в платформе
>* Страница авторизации. При нажатии "Enter" с введенными логином и паролем, имеющихся в системе, происходит  авторизация в платформе
...
>* Страница авторизации. При нажатии "Enter" с введенными логином и паролем, не имеющихся в системе, появляется сообщение об ошибке
...
>* Страница восстановления пароля. При вводе почты, имеющейся в системе, система высылает на почту инструкцию по восстановлению пароля
>* Страница восстановления пароля. При вводе почты, не имеющейся в системе, появляется сообщение об ошибке 
>* Страница восстановления пароля. При попытке восстановления пароля без ввода почты, появляется сообщение об ошибке
...
- [x] Проверить, что модуль авторизации открывается и корректно отображается
...
- [x] Проверить, что пользователь может успешно выйти из системы, и все его данные и сеансы удаляются из системы.
- [x] Проверить, что система правильно обрабатывает и контролирует ошибки доступа, если пользователь пытается получить доступ к страницам и ресурсам, которые не предназначены для его роли и прав.
- [x] Проверить, что что после успешной авторизации пользователь перенаправляется на правильную страницу, которая соответствует его роли и правам доступа
...
- [x] Проверить, что модуль авторизации работает корректно в разных браузерах и их версиях (Кроссбраузерное тестирование)
...
<img title="JmeterScreen" src="img/screens/jmeter_test.png">
<img title="JmeterTree" src="img/screens/jmeter_result_tree.png">
...
Конечно, возможных вариантов проверок очень много, но я постарался охватить наиболее популярные варианты кейсов.
В заключении, хочу сказать, что это одно из самых интересных и лучших тестовых заданий (Если не самое лучшее),
которые я встречал. Выполнять его было интересно и захватывающе (Особенно, когда нашел крутой баг). Хочу поблагодарить
ребят, которые его делали (Тест, сайт). Еще, я пошарил немножко в других модулях 😈. Это очень вдохновляюще 👍.
Вы делаете реально крутой продукт!
